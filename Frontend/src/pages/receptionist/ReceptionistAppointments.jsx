    ...
    <Layout pageTitle="Appointments">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-200 grid grid-cols-2 md:flex flex-wrap gap-3">
          ...
          <button onClick={() => { setFilters({ date: "", doctorId: "", status: "" }); setPage(1); }}
          ...
          <table className="w-full text-left border-separate border-spacing-0">
            <thead>
              <tr className="bg-gray-50">
                {["Patient", "Mobile", "Doctor", "Date", "Time Slot", "Purpose", "Status", "Actions"].map((h) => (
                  <th key={h} className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider border-b border-gray-200">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? <TableLoader colSpan={8} /> : appointments.length === 0 ? <EmptyState colSpan={8} message="No appointments found" /> : (
                appointments.map((apt, index) => (
                  <tr key={apt._id} className={`group transition-colors hover:bg-blue-50/40 ${index % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
                    ...
                    <td className="px-6 py-4 border-b border-gray-100"><StatusBadge status={apt.status} /></td>
                    <td className="px-6 py-4 border-b border-gray-100">
                      <div className="flex items-center gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
                        {apt.status === "scheduled" && (
                          <button onClick={() => handleMarkArrived(apt._id)} className="p-2 text-emerald-600 hover:bg-emerald-100 rounded-lg transition-colors" title="Mark Arrived">
                        ...
                        <button onClick={() => { setSelectedAppointment(apt); setDeleteModal(true); }}
                          className="p-2 text-rose-500 hover:bg-rose-100 rounded-lg transition-colors" title="Delete">
            ...
            appointments.map((apt) => (
              <div key={apt._id} className="p-4 hover:bg-blue-50/30 transition-colors">
                ...
                <div className="mt-3 flex items-center justify-end gap-2 border-t border-gray-100 pt-3">
                  {apt.status === "scheduled" && (
                    <button onClick={() => handleMarkArrived(apt._id)} className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-lg">
                  ...
                  <button onClick={() => { setSelectedAppointment(apt); setDeleteModal(true); }}
                    className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-rose-500 bg-rose-50 hover:bg-rose-100 rounded-lg">
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" value={editForm.purpose} onChange={(e) => setEditForm({ ...editForm, purpose: e.target.value })}
