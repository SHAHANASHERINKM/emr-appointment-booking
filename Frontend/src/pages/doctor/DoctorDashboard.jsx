...
const DoctorDashboard = () => {
  const [stats, setStats] = useState({ total: 0, scheduled: 0, arrived: 0, completed: 0 });
  ...
  const fetchData = async () => {
    try {
      ...
      const [allRes, scheduledRes, arrivedRes, completedRes] = await Promise.all([
        ...
        appointmentApi.getAppointments({ date: today, status: "completed", limit: 1 }),
      ]);
  ...
  const statCards = [
    ...
    { label: "Completed", value: stats.completed, icon: "task_alt", bg: "bg-purple-100", color: "text-purple-600" },
  ];
  ...
  return (
    <Layout pageTitle="Dashboard">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">{new Date().toLocaleDateString("en-GB", { weekday: "long", day: "2-digit", month: "long", year: "numeric" })}</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          ...
          <span className="text-sm text-gray-500">{new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "short" })}</span>
        </div>
          ...
          <table className="w-full text-left border-separate border-spacing-0">
            <thead>
              <tr className="bg-gray-50">
                {["Patient", "Time Slot", "Purpose", "Type", "Status"].map((h) => (
            ...
            <tbody>
              {loading ? (
                ...
                <EmptyState colSpan={5} message="No appointments today" />
              ) : (
            ...
            appointments.map((apt) => (
              <div key={apt._id} className="p-4 hover:bg-blue-50/30 transition-colors">
                ...
                <div className="mt-3 bg-gray-50 rounded-lg p-3 grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    <p className="text-xs text-gray-400">Patient Type</p>
